...
Temp_constr = optimexpr(T_horizon-1,Nseg);

   
for i=1:T_horizon-1
    
   Temp_constr(i,5)= T(i+1,5)== T(i,5)+ (dt/(Ms(j)*Cp))*(...
    ...
    for j=1:4
     Temp_constr(i,j)= T(i+1,j)== T(i,j) +...
                    ...
                    Cres*x(i,j,'RES')-...
                    Qd(i)*x(i,j,'DEM')-...
                    (1-((1-B)^(1/4380))*(Ts(i)-Tgw)*Ms(j)*Cp/3600));
    end
end     
...
tot_Cost(t)= optimexpr(T_horizon);

for i=1:T_horizon
    PVT_gen(t)=nel(t)*G(t)*Apvt*Npvt;
    HP = 0;
    for j=1: Nseg
    HP = HP+((Caw*x(i,j,'AW'))+(Cww1*z1_sink(i,j))+(Cww2*z2_sink(i,j)));
    end
    HP_Cost(t) = ePrice(t)*HP;
    tot_Cost(t) = HP_Cost(t) - PVT_gen(t);
end
